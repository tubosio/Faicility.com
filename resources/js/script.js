document.addEventListener('DOMContentLoaded', () => {
    ...
    const splitClock = document.getElementById('splitClock');
    const splitClockLabel = document.getElementById('splitClockLabel');
    if (splitClock && splitClockLabel) {
        const flaps = Array.from(splitClock.querySelectorAll('.split-flap'));
        const currentDigits = Array(flaps.length).fill(null);

        const setDigit = (flap, oldDigit, newDigit, index) => {
            const top = flap.querySelector('.flap-part.top');
            const bottom = flap.querySelector('.flap-part.bottom');
            const front = flap.querySelector('.flap-dynamic.front');
            const back = flap.querySelector('.flap-dynamic.back');
            ...
            if (flap._frontTimer) {
                window.clearTimeout(flap._frontTimer);
            }
            if (flap._backTimer) {
                window.clearTimeout(flap._backTimer);
            }

            top.textContent = oldDigit;
            bottom.textContent = oldDigit;
            ...
            flap._frontTimer = window.setTimeout(() => {
                bottom.textContent = newDigit;
            }, 320);

            flap._backTimer = window.setTimeout(() => {
                top.textContent = newDigit;
                bottom.textContent = newDigit;
                front.textContent = newDigit;
                back.textContent = newDigit;
        ...
        const renderClock = () => {
            ...
            const allDigits = `${hour}${minute}${second}`;

            allDigits.split('').forEach((digit, index) => {
                const flap = flaps[index];
                if (flap) {
                    setDigit(flap, currentDigits[index], digit, index);
                }
            });

            splitClockLabel.textContent = now.toLocaleString('nl-NL', {
                weekday: 'long',
                day: 'numeric',
                month: 'long',
                hour: '2-digit',
